...
$tmp = tempnam(sys_get_temp_dir(), 'cap_') . '.jpg';
...
$cmd = escapeshellcmd($cfg['python_bin']) . ' ' .
    escapeshellarg($cfg['tools_dir'] . '/face_check.py') . ' ' .
    '--image ' . escapeshellarg($tmp) . ' ' .
    '--db ' . escapeshellarg($cfg['faces_db_dir']) . ' ' .
    '--tolerance ' . escapeshellarg((string)$cfg['tolerance']);
